...
namespace LiveLisp.Core.Types
{
    ...
    [Serializable]
#endif
    public class BigInteger : ILispNumber {
       ...
       // [CLSCompliant(false)]
        ...
        public static BigInteger Create(byte[] v) {
            ...
            return Create(v, isNegative , byteCount);
        }
        public static BigInteger CreateUnsigned(byte[] v)
        {
            if (v.Length == 0) return Create(0);

           
            return Create(v, false, v.Length);
        }

        private static BigInteger Create(byte[] v, bool isNegative, int byteCount)
        {
       ...
       // [CLSCompliant(false)]
        ...
        public static  BigInteger Create(object self)
        {
            if (self is BigInteger)
            {
                return (BigInteger)self;
            }
            else if (self is ValueType)
            {
                ...
                if (!type.IsEnum)
                {
                    ...
                    {
                        case TypeCode.Byte:
                            return Create((int)self);
                        case TypeCode.Int16:
                            return Create((int)self);
                        case TypeCode.Int32:
                            return Create((int)self);
                        case TypeCode.Int64:
                            return Create((Int64)self);
                        case TypeCode.Double:
                            return Create((double)self);
                        case TypeCode.Single:
                            return Create((double)self);
                        case TypeCode.UInt16:
                            return Create((uint)self);
                        case TypeCode.UInt32:
                            return Create((uint)self);
                        case TypeCode.UInt64:
                            return Create((ulong)self);
                    }
                }
            }

            throw new InvalidCastException("cannont cast from type " + self.GetType() + " to BigInteger.");
        }
       ...
       // [CLSCompliant(false)]
        ...
        public static BigInteger operator <<(BigInteger x, Int32 shift) {
            if (object.ReferenceEquals(x, null)) {
                throw new ArgumentNullException("x");
            }
            if (shift == 0) return x;
        ...
        public string ToString(uint radix)
        {
            ...
            if (radix > 36) {
                throw new ArgumentOutOfRangeException("radix", 
                    ...
                    radix, 
#endif
                MathResources.RadixGreaterThan36);
            }
       ...
       // [CLSCompliant(false)]
        ...
        /// <summary>
        /// Parses the specified base10literal.
        /// TODO: Slow as hell
        /// </summary>
        /// <param name="base10literal">The base10literal.</param>
        /// <returns></returns>
        internal static bool TryParse(string base10literal, out BigInteger result)
        {
            result = null;
            var pow = base10literal.Length - 1;
            ...
            var base16literal = RadixConverter.Convert(10, 16, base10literal);
            ...
            List<UInt32> dataproto = new List<uint>();

            List<string> temp = new List<string>();

            int fulloctets = base16literal.Length / 8;
            int partialoctetlength = base16literal.Length % 8;

            if (partialoctetlength != 0)
            {
              uint td ;
              if (!UInt32.TryParse(base16literal.Substring(0, partialoctetlength), NumberStyles.HexNumber, null, out td))
              {
                  return false;
              }
                dataproto.Insert(0, td);
                temp.Insert(0, base16literal.Substring(0, partialoctetlength));
            }
            uint td1;
            for (var i = 0; i < fulloctets; i++)
            {
                if (!UInt32.TryParse(base16literal.Substring(partialoctetlength + i * 8, 8), NumberStyles.HexNumber, null, out td1))
                {
                    return false;
                }
                dataproto.Insert(0, td1);
                temp.Insert(0, base16literal.Substring(partialoctetlength + i * 8, 8));
            }



                result = new BigInteger(sign, dataproto.ToArray());
                return true;
        }
        ...
        BigInteger ILispNumber.BigInteger
        {
            get { throw new NotImplementedException(); }
        }
